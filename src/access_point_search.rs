...
pub fn search(lat_min:f64, lat_max:f64, long_min:f64, long_max:f64, ssid: String, api_key: String) -> Result<(f64, f64), Error> {
    ...
    println!("Searching...");
    let raw_data = client.get(&request_url).header("Authorization",api_key).send()?.text()?.to_owned();

    let resp: Output = serde_json::from_str(raw_data.as_str()).unwrap();
